...
<body>
	...
	<div id="wrapper">
        ...
        <label for="exampleEmail1">Kode Invoice</label>
        <input type="text" name="invoice_id" placeholder="Masukkan Kode Pemesanan" required/>
        </div>
        <div class="form-group">
        <button type="submit" class="btn btn-info text-uppercase" name="submit">Cek Status Pemesanan</button>
        </div>
        </form>
        <?php
            if(isset($_POST['submit'])) {
                $kode = trim($_POST['invoice_id']);
                $query = mysqli_query($koneksi,"SELECT * FROM orders WHERE invoice = '$kode'");
                $get = mysqli_fetch_array($query);
                $jumlah = mysqli_num_rows($query);
                if($jumlah == 0) {
                    echo "<div class='alert alert-danger'><b>Gagal</b> Tidak ada Pemesanan di Kode yang anda masukkan";
                } elseif(!$kode) {
                    echo "<div class='alert alert-danger'><b>Gagal</b> Silahkan input kembali";
                } else {
        ?>
                ...
                <tr>
                <td><center><?php echo $get['invoice'];?></center></td>
                <td><center><?php echo $get['nama_barang'];?></center></td>
                <td><center><?php echo $get['nama'];?></center></td>
                <td><center><?php echo $get['alamat'];?></center></td>
                <td><center><?php echo $get['kota'];?></center></td>
                <td><center>Rp <?php echo number_format($get['biaya']); ?></center></td>
                <td><center><?php echo $get['status_pesanan'];?></center></td>
                </tr>
                </table>
                <div class="alert alert-info">
                    Silahkan lakukan pembayaran melalui kontak dibawah ini untuk konfirmasi pembayaran <a href="https://wa.me/628561333111?text=Saya%20ingin%20konfirmasi%20pembayaran%20invoice%20<?php echo $get['invoice'];?>">disini</a>
                </div>
            <?php
                }
            }
