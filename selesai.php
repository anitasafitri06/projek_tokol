...
<body>
    ...
    <?php
			if($_POST['finish']){
				...
				$query = mysqli_query($koneksi, "SELECT * FROM orders WHERE nama = '$nama_lengkap' AND status_pesanan = 'Menunggu Pembayaran'");
				if(mysqli_num_rows($query)) {
					echo "<div class='alert alert-danger'><b>Gagal</b>! Tidak dapat memproses pemesanan dikarenakan ada pemesanan yang belum diselesaikan pembayarannya! Silahkan lakukan pembayaran atau hubungi admin!</div>";
				} elseif(!$nama_lengkap || !$email || !$alamat || !$kodepos) {
					echo "<div class='alert alert-danger'><b>Gagal</b>! Silahkan mengisi ulang data!</div>";
				} else {
					$invoice = "DMSN".rand(1111,9999);
					...
					if($simpan == true) {
						echo "<div class='alert alert-success'><b>Berhasil</b>! Silahkan lakukan pembayaran melalui kontak dibawah ini untuk konfirmasi pembayaran <a href='https://wa.me/628561333111?text=Saya%20ingin%20konfirmasi%20pembayaran%20invoice%20$invoice'>disini</a></div>";
					} else {
